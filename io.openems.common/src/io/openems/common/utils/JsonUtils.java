...
public static ZonedDateTime getAsZonedDateWithCurrentTime(JsonElement element, String memberName, ZoneId timezone)
            throws OpenemsNamedException {
        var date = JsonUtils.getAsString(element, memberName).split("-");
        try {
            var year = Integer.parseInt(date[0]);
            ...
            return ZonedDateTime.of(year, month, day, now.getHour(), now.getMinute(), now.getSecond(), now.getNano(),
                    timezone);
        } catch (NumberFormatException | ArrayIndexOutOfBoundsException e) {
            throw OpenemsError.JSON_NO_DATE_MEMBER.exception(memberName, element.toString(), e.getMessage());
        }
